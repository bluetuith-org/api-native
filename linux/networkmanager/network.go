...
func Initialize() (*NetManager, ac.AppCapabilities, *ac.Error) {
	manager, err := nm.NewNetworkManager()
	if err != nil {
		return nil, ac.CapabilityNone,
			ac.NewError(ac.CapabilityNetwork, err)
	}
	...
	return network, ac.CapabilityNetwork, nil
}
...
func (n connectionSettings) toMap() map[string]map[string]interface{} {
	...
	if n.ConnectionType == bluetooth.DUN {

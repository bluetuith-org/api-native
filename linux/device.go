...
// Use via (*Session).Device(deviceAddress) only.
type Device struct {
	b    *BluezSession
...
func (d *Device) ConnectProfile(profileUUID string) error {
	if _, err := d.check(); err != nil {
		return err
	}

	if err := d.callDevice("ConnectProfile", 0, profileUUID).Store(); err != nil {
		return fault.Wrap(err,
			fctx.With(context.Background(),
...
// to an adapter, using a specific Bluetooth profile UUID .
func (d *Device) DisconnectProfile(profileUUID string) error {
	if _, err := d.check(); err != nil {
		return err
	}

	if err := d.callDevice("DisconnectProfile", 0, profileUUID).Store(); err != nil {
		return fault.Wrap(err,
			fctx.With(context.Background(),
...
func (d *Device) convertAndStoreObjects(values map[string]dbus.Variant) error {
	...
	*/
	...
	if p, err := d.batteryPercentage(); err == nil {
		device.BatteryPercentage = int(p)
	}

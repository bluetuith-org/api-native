...
// obex session.
func (o *Obex) Initialize(auth bluetooth.ReceiveFileAuthHandler) (ac.AppCapabilities, *ac.Error) {
	var capabilities ac.AppCapabilities

	serviceNames, err := dbh.ListBusNames(o.SessionBus)
	if err != nil {
		return capabilities,
			ac.NewError(ac.CapabilitySendFile|ac.CapabilityReceiveFile, err)
	}
	...
	return capabilities,
		ac.NewError(
			ac.CapabilitySendFile|ac.CapabilityReceiveFile,
...
SetupAgent:
	...
	capabilities = ac.CapabilitySendFile
	if err := setupAgent(o.SessionBus, auth); err != nil {
		return capabilities,
			ac.NewError(ac.CapabilityReceiveFile, err)
	}

	capabilities |= ac.CapabilityReceiveFile
...
func (o *Obex) parseSignalData(signal *dbus.Signal) {
	...
	case dbh.DbusSignalPropertyChangedIface:
		...
		case dbh.ObexTransferIface:
			...
			events.FileTransferEvent(events.EventActionUpdated).Publish(transferData)
		}
	}
}

...
// BluezAgent describes an Bluez agent connection.
...
func (b *BluezAgent) DisplayPasskey(devicePath dbus.ObjectPath, passkey uint32, entered uint16) *dbus.Error {
	if !b.initialized {
		return nil
	}
...
func (b *BluezAgent) RequestConfirmation(devicePath dbus.ObjectPath, passkey uint32) *dbus.Error {
	if !b.initialized {
		return nil
	}
...
func (b *BluezAgent) RequestAuthorization(devicePath dbus.ObjectPath) *dbus.Error {
	if !b.initialized {
		return nil
	}
...
func (b *BluezAgent) AuthorizeService(devicePath dbus.ObjectPath, uuidstr string) *dbus.Error {
	if !b.initialized {
		return nil
	}
...
func (b *BluezAgent) Cancel() *dbus.Error {
	if !b.initialized && b.ctx != nil {
		b.cancel()
	}
...
func (b *BluezAgent) Release() *dbus.Error {
	if !b.initialized {
		return nil
	}
...
func setupAgent(systemBus *dbus.Conn, authHandler bluetooth.SessionAuthHandler) error {
	if authHandler == nil {
...
func removeAgent() error {
	...
	return agent.callAgentManager("UnregisterAgent", dbh.BluezAgentPath).Store()
}

// callAgentManager calls the AgentManager1 interface with the provided arguments.
func (b *BluezAgent) callAgentManager(method string, args ...interface{}) *dbus.Call {

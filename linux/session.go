...
import (
	...
	events "github.com/bluetuith-org/api-native/api/events"
...
func (b *BluezSession) Start(
	authHandler bluetooth.SessionAuthHandler,
	_ ...string,
) (ac.Collection, error) {
	var capabilities ac.AppCapabilities

	var ce ac.Errors

	systemBus, err := dbus.SystemBus()
	if err != nil {
		return ac.NilCollection(),
			fault.Wrap(err,
	...
	*b = BluezSession{
	...
	if err := setupAgent(systemBus, authHandler); err != nil {
		return ac.NilCollection(),
			fault.Wrap(err,
	...
	capabilities |= ac.CapabilityConnection | ac.CapabilityPairing

	obexcap, cerr := b.obex().Initialize(authHandler)
	if cerr != nil {
		ce.Append(cerr)
	}
	...
	return ac.NewCollection(capabilities, ce), nil
}
...
func (b *BluezSession) MediaPlayer(deviceAddress bluetooth.MacAddress) bluetooth.MediaPlayer {
	return &MediaPlayer{systemBus: b.systemBus, Address: deviceAddress}
}
...
func (b *BluezSession) mediaPlayer() *MediaPlayer {
	return &MediaPlayer{systemBus: b.systemBus}
}
...
func (b *BluezSession) parseSignalData(signal *dbus.Signal) {
	...
	case dbh.DbusSignalPropertyChangedIface:
		...
		case dbh.BluezAdapterIface:
			var adapter bluetooth.AdapterEventData

			if err := dbh.DecodeVariantMap(propertyMap, &adapter); err != nil {
				dbh.PublishSignalError(err, signal,
					"Bluez event handler error",
					"error_at", "pchanged-adapter-decode",
				)

				return
			}

			address, ok := dbh.PathConverter.Address(dbh.DbusPathAdapter, signal.Path)
			if !ok {
			...
			updated, err := b.store.UpdateAdapter(address, adapter)
			if err != nil {
				dbh.PublishSignalError(err, signal,
					"Bluez event handler error",
			...
			events.AdapterEvent(events.EventActionUpdated).Publish(updated)

		case dbh.BluezDeviceIface:
			var device bluetooth.DeviceEventData

			if err := dbh.DecodeVariantMap(propertyMap, &device); err != nil {
				dbh.PublishSignalError(err, signal,
					"Bluez event handler error",
					"error_at", "pchanged-device-decode",
				)

				return
			}

			address, ok := dbh.PathConverter.Address(dbh.DbusPathDevice, signal.Path)
			if !ok {
				dbh.PublishSignalError(errorkinds.ErrDeviceNotFound, signal,
					"Bluez event handler error",
					"error_at", "pchanged-device-address",
				)

				return
			}

			updated, err := b.store.UpdateDevice(address, device)
			if err != nil {
				dbh.PublishSignalError(err, signal,
					"Bluez event handler error",
					"error_at", "pchanged-device-update",
				)

				return
			}

			events.DeviceEvent(events.EventActionUpdated).Publish(updated)
		...
		case dbh.BluezMediaPlayerIface:
			...
			events.MediaEvent(events.EventActionUpdated).Publish(bluetooth.MediaEventData{
				Address:   address,
		...
		case dbh.BluezBatteryIface:
			...
			device.BatteryPercentage = int(percentage)

			address, ok := dbh.PathConverter.Address(dbh.DbusPathDevice, signal.Path)
			if !ok {
				dbh.PublishSignalError(errorkinds.ErrDeviceNotFound, signal,
					"Bluez event handler error",
					"error_at", "pchanged-batterypct-address",
				)
			...
			if err != nil {
				dbh.PublishSignalError(err, signal,
					"Bluez event handler error",
					"error_at", "pchanged-batterypct-update",
				)
	...
	case dbh.DbusSignalInterfacesAddedIface:
		...
		for iftype := range nestedPropertyMap {
			...
			case dbh.BluezAdapterIface:
				...
				events.AdapterEvent(events.EventActionAdded).
			...
			case dbh.BluezDeviceIface:
				...
				events.DeviceEvent(events.EventActionAdded).
			...
			case dbh.BluezBatteryIface:
				...
				if v, ok := nestedPropertyMap[iftype]["Percentage"]; ok {
					if p, ok := v.Value().(byte); ok {
						percentage = int(p)
					}
				}
				...
				if err != nil {
					dbh.PublishSignalError(err, signal,
						"Bluez event handler error",
						"error_at", "padded-batterypct-update",
					)
	...
	case dbh.DbusSignalInterfacesRemovedIface:
		...
		for _, ifaceName := range ifaceNames {
			...
			case dbh.BluezAdapterIface:
				...
				events.AdapterEvent(events.EventActionRemoved).Publish(adapter)
			...
			case dbh.BluezDeviceIface:
				...
				events.DeviceEvent(events.EventActionRemoved).Publish(device)
			...
			case dbh.BluezBatteryIface:
				...
				if !ok {
					dbh.PublishSignalError(errorkinds.ErrDeviceNotFound, signal,
						"Bluez event handler error",
						"error_at", "premoved-batterypct-address",
					)
				...
				if err != nil {
					dbh.PublishSignalError(err, signal,
						"Bluez event handler error",
						"error_at", "premoved-batterypct-update",
					)
				}

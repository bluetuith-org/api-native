...
func (s *SessionStore) UpdateAdapter(
	...
	adapterEvent bluetooth.AdapterEventData,
	...
	adapterEvent.Address = adapterAddress
	adapter.AdapterEventData = adapterEvent
...
func (s *SessionStore) UpdateDevice(deviceAddress bluetooth.MacAddress, deviceEvent bluetooth.DeviceEventData) (bluetooth.DeviceEventData, error) {
	device, ok := s.devices.Load(deviceAddress)
	if !ok {
	...
	deviceEvent.Address = deviceAddress
